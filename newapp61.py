...
from utils61 import (
    ...
    export_csv_bytes_rows, export_html, render_table_html,
...
if contract_type == "Host":
    ...
    if "host_df" in st.session_state:
        ...
        # Single-row CSV of inputs + results
        source_df = df.copy()
            ...
            if st.button("Generate Production Costs", key="generate_contractual"):
                ...
                if errs:
                    ...
                    results = calculate_production_contractual(
                        ...
                        supervisor_salaries=supervisor_salaries,
                        effective_pct=100.0,  # fixed cost basis
                        customer_covers_supervisors=customer_covers_supervisors,
                        region=region,
                        ...
                        dev_rate=None,  # handled inside module
                    ...
                    prod_df = pd.DataFrame([{k: (None if r.get(k) is None else (round(float(r.get(k)), 4) if isinstance(r.get(k), (int, float)) else r.get(k)))
        ...
        for i in range(int(num_lines)):
            with st.expander(f"Product line {i+1}", expanded=(i == 0)):
                ...
                lines.append({
                    ...
                    "units": int(units_requested), "deadline": deadline,
                    "pris_per_item": int(pris_per_item), "mins_per_item": float(minutes_per_item),
                })
        ...
        if st.button("Generate Ad-hoc Costs", key="generate_adhoc"):
            ...
            if errs:
                ...
                result = calculate_adhoc(
                    ...
                    effective_pct=100.0,
                    customer_covers_supervisors=customer_covers_supervisors,
                    region=region, customer_type="Commercial",
                    apply_vat=True, vat_rate=20.0, dev_rate=None,
                    today=date.today(), lock_overheads=lock_overheads,
                    employment_support=employment_support,
                )
...
# Results display for Production (if any)
if "prod_df" in st.session_state and isinstance(st.session_state["prod_df"], pd.DataFrame):
    st.markdown(render_table_html(st.session_state["prod_df"]), unsafe_allow_html=True)