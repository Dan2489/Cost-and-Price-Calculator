...
def calculate_production_contractual(
    ...
    *,
    ...
    if customer_covers_supervisors:
        ...
        overheads_weekly = (shadow_salary / 52.0) * CFG.OVERHEAD_PCT
    else:
        if st.session_state.get("lock_overheads"):
            highest = max(supervisor_salaries) if supervisor_salaries else 0.0
            overheads_weekly = (highest / 52.0) * CFG.OVERHEAD_PCT
        else:
            overheads_weekly = sum(((s / 52.0) * CFG.OVERHEAD_PCT) for s in supervisor_salaries)

    # Development charge (applies but not shown separately)
    ...
    for idx, it in enumerate(items):
        ...
        monthly_total = None
        if units_for_pricing and unit_cost_ex_vat:
...
def calculate_adhoc(
    ...
    *,
    ...
    for ln in lines:
        ...
        unit_cost_inc_vat = unit_cost_ex_vat * (1 + (float(vat_rate) / 100.0)) if customer_type == "Commercial" else unit_cost_ex_vat
