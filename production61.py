# production61.py
# Production cost models based on Instructor costs only (overheads fixed at 61%)
...
def labour_minutes_budget(num_pris: int, hours: float) -> float:
    """Total labour minutes available = prisoners × hours × 60."""
...
def weekly_overheads_total(
    *,
    ...
    """Calculate weekly overheads (61% of instructor costs or Band 3 shadow)."""
    if not customer_covers_instructors:
        weekly_inst = sum((s / 52.0) * (float(effective_pct) / 100.0) for s in instructor_salaries)
        return weekly_inst * 0.61
    else:
        shadow_weekly = BAND3_SHADOW["weekly"]
        return shadow_weekly * 0.61
...
def calculate_production_contractual(
    ...
    *,
    ...
    overheads_weekly = weekly_overheads_total(
        num_instructors=num_instructors,
        customer_covers_instructors=customer_covers_instructors,
        instructor_salaries=instructor_salaries,
        effective_pct=effective_pct,
    )

    inst_weekly_total = (
        sum((s / 52.0) * (float(effective_pct) / 100.0) for s in instructor_salaries)
        if not customer_covers_instructors
        else 0.0
    )

    prisoner_weekly_total = num_prisoners * prisoner_salary
    weekly_fixed_total = inst_weekly_total + overheads_weekly
...
def calculate_adhoc(
    ...
    *,
    ...
    overheads_weekly = weekly_overheads_total(
        num_instructors=len(instructor_salaries),
        customer_covers_instructors=customer_covers_instructors,
        instructor_salaries=instructor_salaries,
        effective_pct=effective_pct,
    )
