# production61.py
# Production pricing (contractual + ad-hoc) with 61% overheads method.
...
from utils61 import labour_minutes_budget, overheads_weekly_61, format_currency
...
def calculate_production_contractual(
    ...
    *,
    ...
    # Overheads (weekly) via 61% method
    overheads_weekly, _basis = overheads_weekly_61(
    ...
    # Dev charge applies only for Commercial
    dev_weekly_total = overheads_weekly * float(dev_rate) if customer_type == "Commercial" else 0.0
    # Instructor weekly salaries (charged only if not provided by customer)
    inst_weekly_total = (
...
def calculate_adhoc(
    ...
    *,
    ...
    overheads_weekly, _ = overheads_weekly_61(
        supervisor_salaries=supervisor_salaries,
        customer_covers_supervisors=customer_covers_supervisors,
        region=region,
        effective_pct=effective_pct,
...
def production_summary_table(df: pd.DataFrame) -> str:
    ...
    for _, row in df.iterrows():
        ...
        for col in cols:
            ...
            if isinstance(val, (int, float)) and pd.notna(val):
                if "£" in col:
                    tds.append(f"<td>{format_currency(val)}</td>")
                else:
