...
def export_html(host_df=None, prod_df=None, title="Quote", extra_note=None) -> BytesIO:
    css = "<style>table{width:100%;border-collapse:collapse;}th,td{border:1px solid #b1b4b6;padding:6px;text-align:left;}th{background:#f3f2f1;}</style>"
    ...
    if host_df is not None: parts += ["<h3>Host Costs</h3>", host_df.to_html(index=False, border=1)]
    if prod_df is not None: parts += ["<h3>Production Items</h3>", prod_df.to_html(index=False, border=1)]
...
def render_table_html(df: pd.DataFrame) -> str:
    ...
    for col in df2.columns:
        if "£" in str(col):
            try: df2[col] = pd.to_numeric(df2[col], errors="coerce").map(lambda x: fmt_currency(x) if pd.notna(x) else "")
